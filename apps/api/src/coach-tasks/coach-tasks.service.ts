...
@Injectable()export class CoachTasksService extends ResourceService<'coachTask'>{
 ...
 protected async validate(data:Partial<import('../data/entities').CoachTask>,tx?:import('../data/database.service').Transaction){await super.validate(data,tx);if(data.coachId&&this.tenant.role===Role.COACH&&data.coachId!==this.tenant.userId)throw new ForbiddenException('Coaches can only assign tasks to themselves');}
