...
@Injectable() export class MembershipsService {
 constructor(private readonly repository: MembershipsRepository, private readonly db: DatabaseService, private readonly tenant: TenantContext) {}
 ...
 async invite(dto: InviteMemberDto) {
   ...
   return this.db.transaction(async tx => {
     ...
     return this.repository.create({ userId: null, invitationEmail: dto.email.toLowerCase(), invitationStatus: 'PENDING', role: dto.role, deletedAt: null }, tx);
   });
 }
