...
@Injectable()export class CampaignsService extends ResourceService<'campaign'>{
 ...
 protected filters(q:import('../common/dto/resource-query.dto').ResourceQueryDto){return {...super.filters(q),archivedAt:null};}
 async pause(id:string){return this.db.transaction(async tx=>{const row=await this.db.require('campaign',this.repository.organizationId,id,tx);if(row.archivedAt||![CampaignStatus.RUNNING,CampaignStatus.SCHEDULED].includes(row.status))throw new ConflictException('Only scheduled or running campaigns can be paused');return this.repository.update(id,{status:CampaignStatus.PAUSED},tx);});}
 async resume(id:string){const row=await this.get(id);if(row.status!==CampaignStatus.PAUSED||row.archivedAt)throw new ConflictException('Only paused campaigns can be resumed');return this.schedule(id);}
