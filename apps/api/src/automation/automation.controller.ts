...
@ApiTags('automations') @ApiBearerAuth() @Roles(Role.OWNER,Role.ADMIN) @Plan('automation') @Controller('automations')export class AutomationController{
 ...
 @Post(':id/run') @ApiOperation({summary:'Run a manual rule for a student'}) run(@Param('id')id:string,@Body()dto:RunAutomationDto){return this.service.run(id,dto.studentId);}
